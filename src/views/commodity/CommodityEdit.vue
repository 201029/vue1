<template>
  <div class="commodityEdit">
    <h1 class="tatle">商品修改</h1>
      ...
      <el-form status-icon ref="ruleForm" label-width="100px" class="demo-ruleForm">
        ...
        <div v-show="!flag" class="table-box">
          ...
          <el-table :data="tableData" border style="width:642px">
            ...
            <el-table-column @click="dialogFormVisible=true" label="操作/添加" width="100">
        ...
        <el-form-item>
          <el-button @click="goodsEstablish" type="primary">保存修改</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
export default {
  ...
  //生命钩子 一进页面就调用 用于 回填数据
  created() {
    /* 获取本地数据 */

    let con = JSON.parse(window.sessionStorage.getItem("editList")); //用于一次性回填没有回填成功的  二次处理后回填

    this.merchandise = JSON.parse(window.sessionStorage.getItem("editList")); //一次性回填全部

    console.log(con.id);
    this.id = con.id; //获取id 用于修改内容
    
    var string = con.goodscategory;
    console.log(string);
    var obj2 = string.split(""); //字符串转化为数组
    console.log(obj2);

    let arr = obj2.filter(function(v) {
      return v !== "/";
    });
    console.log("arr", arr); //
    ...
    this.merchandise.goodscategory = chunk(arr2, 2); //商品分类   ["饮品", "热饮", "咖啡"];

    this.merchandise.goodsfeature = JSON.parse(con.goodsfeature); // 特色数据处理和再次回填

    let standard = JSON.parse(con.standard); // 用于单规格和多规格的处理回填

    if (standard[0].goodsstandard === "单规格") {
      /* 单规格 */
      this.merchandise.goodsstandard = "单规格";
      this.merchandise.packingexpense = standard[0].packingexpense; //包装费
      this.merchandise.goodsPrice = standard[0].goodsPrice; // 起送价
    } else {
      /* 多规格 */
      this.merchandise.goodsstandard = "多规格";
      this.tableData = standard; //多规格 表格
    }
  }
};
</script>
